...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      // brandsId:{
      //   handler: function(){
      //     this.brandsId = this.$store.state.edit.brandsId;
    ...
    methods: {
      initBrand(){
        ...
        let len = this.brandConfig.link_value.length;
        if(this.brandConfig.link_value.substring(len-1)!=',' && len>4){
          this.brandConfig.link_value += ','
        }
      ...
      saving(){
        ...
        let len = this.brandConfig.link_value.length;
        if(len){
          this.brandConfig.link_value = this.brandConfig.link_value.slice(0,len-1);
          this.brandConfig.link_value = (Array.from(new Set(this.brandConfig.link_value.split(',')))).join(',');
        }
      ...
      handleSelectionChange(val) {
        if(val.length){
          this.brandConfig.link_value += this.getArrByKey('id',val).join(',') + ',';
        }
        // this.shopConfig.link_coupon = this.getArrByKey('id',val).join(',')
      },
      paging(val) {
        // console.log(this.brandsId);
        // console.log(Array.from(new Set(this.flattenDepth(this.brandsId))));
