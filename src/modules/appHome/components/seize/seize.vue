...
<script>
  ...
  export default {
    ...
    methods: {
      isRegFn1(){
        let isReg = this.seizeConfig.image_url !=="" ? true : false;
        return isReg
      },
      isRegFn2(){
        let isReg = this.seizeConfig.link_type !=="" ? true : false;
        return isReg
      },
      isRegFn3(){
        let isReg = this.seizeConfig.link_value !=="" ? true : false;
        return isReg
      },
      saving(){
        if(this.seizeConfig.is_open){
          ...
          let message = '保存成功'
          let message1 = '占位图不能为空';
          let message2 = '次级页面不能为空';
          let message3 = '次级页面不能为空';
          let type = 'error';
