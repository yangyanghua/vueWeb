...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      saving(){
        ...
        if(!isReg1){
          ...
          delete this.entryConfig.entryAdd;
          delete this.entryConfig.toPopupIndex;
          //发送到vuex进行公共处理
      ...
      addContentBox(){
        if(this.entryConfig.sub_entry.length >= 10){
          this.tips('error','至多添加10个快捷入口');
          return false;
        }
    ...
    mounted() {
      ...
      // this.entryConfig = this.objCopy(this.$store.state.edit.entry)
      this.entryConfig = Object.assign(this.entryConfig, this.$store.state.edit.entry);
      console.log(this.entryConfig);

      if(this.entryConfig.sub_entry && this.entryConfig.sub_entry.length>0){
        this.toPopupIndex = this.$store.state.edit.toPopupIndexEntry || 0;
